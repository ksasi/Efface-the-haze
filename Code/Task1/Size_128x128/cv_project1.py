...
from kornia.losses import ssim, psnr
...
def gen_image_list(path):
  ...
  img_path_128 = [cv2.resize(img, (128, 128)) for img in img_list_bgr]
  img_list = [cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in img_path_128]
...
def calc_avg_psnr(gt_list, hz_list, lambda_func, flag = 'id'):
  ...
  if flag == 'id':
    for idx,item in enumerate(hz_list):
      ...
      psnr_val = psnr(torch.from_numpy(gt_img.astype('float')/255).permute(2,0,1).unsqueeze(0), torch.from_numpy(dehazed_img[0:gt_img.shape[0], 0:gt_img.shape[1], :].astype('float')/255).permute(2,0,1).unsqueeze(0), 1).detach().item()
      psnr_list.append(psnr_val)
      #####ssim_val = structural_similarity(gt_img, dehazed_img[0:gt_img.shape[0], 0:gt_img.shape[1], :], multichannel = True)
      ssim_val = torch.mean(ssim(torch.from_numpy(gt_img.astype('float')/255).permute(2,0,1).unsqueeze(0), torch.from_numpy(dehazed_img[0:gt_img.shape[0], 0:gt_img.shape[1], :].astype('float')/255).permute(2,0,1).unsqueeze(0), window_size=11)).detach().item()
...
img_list_od_hz[0].shape

"""### Gaussian Pyramid - DID NOT WORK"""

img_path_list = [img_path for img_path in path_id_hz.iterdir()]
img_list_bgr = [cv2.imread(str(img_path)) for img_path in sorted(img_path_list)]

img_list_bgr[0].shape
...
for i in range(1,6):
    ...
    #print(ls_.shape)
    #print(LS[i].shape)
    ls_ = cv2.add(ls_[0:lpA[i].shape[0], 0:lpA[i].shape[1], 0:lpA[i].shape[2]], lpA[i])
